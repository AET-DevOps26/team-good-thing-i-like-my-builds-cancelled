...
public class TimetableService {
	...
	public List<Station> findByName(String name) throws IOException, InterruptedException {
		HttpRequest request = HttpRequest.newBuilder()
				.uri(URI.create(
						"https://www.bahn.de/web/api/reiseloesung/orte?suchbegriff=" + name + "&typ=ALL&limit=10"))
				.GET().build();
	...
	public List<TrainConnection> findConnections(String from, String to, OffsetDateTime time)
		...
		HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://www.bahn.de/web/api/angebote/fahrplan"))
	...
	private String ungzip(HttpResponse<byte[]> response) throws IOException {
		...
		boolean gzip = response.headers().firstValue("Content-Encoding").map(v -> v.equalsIgnoreCase("gzip"))
				.orElse(false);
